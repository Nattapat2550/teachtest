...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-6xl w-full mx-auto p-6 lg:p-10 mt-10 space-y-8 animate-fade-in pb-20">
      <div className="bg-white  rounded-md shadow-sm border border-gray-200  p-6 lg:p-8">
        <h2 className="text-2xl font-bold border-b border-gray-200  pb-4 mb-6 text-gray-900 ">การตั้งค่าบัญชี</h2>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="flex flex-col items-center space-y-4 shrink-0">
            ...
            <label className="cursor-pointer bg-canvas  border border-gray-200  text-gray-700  py-2 px-5 rounded-md shadow-sm hover:bg-gray-100 dark:hover:bg-gray-600 transition text-sm font-medium">
              เปลี่ยนรูปโปรไฟล์
          ...
          <form onSubmit={handleUpdateProfile} className="flex-1 space-y-5">
            <div>
              ...
              <input type="text" value={profile.username} onChange={(e) => setProfile({...profile, username: e.target.value})} className="block w-full border border-gray-300  rounded-md p-3 bg-canvas  text-gray-900  outline-none focus:ring-2 focus:ring-blue-500" required />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-medium text-gray-700  mb-1">ชื่อจริง</label>
                <input type="text" value={profile.first_name} onChange={(e) => setProfile({...profile, first_name: e.target.value})} className="block w-full border border-gray-300  rounded-md p-3 bg-canvas  text-gray-900  outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700  mb-1">นามสกุล</label>
                <input type="text" value={profile.last_name} onChange={(e) => setProfile({...profile, last_name: e.target.value})} className="block w-full border border-gray-300  rounded-md p-3 bg-canvas  text-gray-900  outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700  mb-1">เบอร์โทรศัพท์</label>
              <input type="tel" value={profile.tel} onChange={(e) => setProfile({...profile, tel: e.target.value})} className="block w-full border border-gray-300  rounded-md p-3 bg-canvas  text-gray-900  outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
      ...
      <div className="bg-white  rounded-md shadow-sm border border-red-200 /50 p-6 lg:p-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <h3 className="text-xl font-bold text-red-600 ">Danger Zone</h3>
          <p className="text-sm text-gray-500  mt-1">ลบบัญชีของคุณอย่างถาวร (บัญชีจะถูกลบหลังจาก 30 วัน)</p>
        </div>
