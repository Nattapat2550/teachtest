...
export default function PackageDetail() {
 ...
 if (!pkg) return <div className="text-center mt-20 ">ไม่พบแพ็กเกจเรียน</div>;

 return (
 <div className="max-w-7xl w-full mx-auto p-6 lg:p-10 mt-8 mb-20">
 <div className="bg-canvas rounded-md p-8 shadow-md border border-outline flex flex-col md:flex-row gap-8">
 
 <div className="w-full md:w-1/3">
 <div className="aspect-square bg-canvas rounded-md overflow-hidden shadow-sm relative">
 {isOwned && (
 <div className="absolute top-2 left-2 bg-green-500 text-white text-xs px-3 py-1.5 rounded-full z-10 font-bold shadow-md">
 เป็นเจ้าของแล้ว
 </div>
 )}
 {pkg.cover_image ? <img src={pkg.cover_image} className="w-full h-full object-cover" alt="Cover" /> : <div className="w-full h-full flex items-center justify-center text-muted">ไม่มีรูปปก</div>}
 </div>
 </div>
 ...
 {/* เอา text-purple ออก */}
 <h1 className="text-3xl font-black text-ink mb-4">{pkg.title}</h1>
 <p className="text-muted mb-6">{pkg.description}</p>
 
 <div className="bg-canvas p-6 rounded-md border border-outline ">
 <div className="flex justify-between items-center mb-4">
 <div className="text-3xl font-black text-primary">฿ {Number(pkg.price).toLocaleString()}</div>
 ...
 <div className="mt-12 bg-canvas rounded-md p-8 shadow-sm border border-outline ">
 <h3 className="text-xl font-bold mb-6 ">คอร์สทั้งหมดที่จะได้รับในแพ็กเกจนี้:</h3>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 {pkg.included_courses.map((c: any) => (
 <div key={c.id} className="flex items-center gap-4 p-4 bg-canvas rounded-md border border-outline hover:border-blue-300 transition-colors">
 {c.cover_image ? (
 <img src={c.cover_image} alt={c.title} className="w-20 h-14 object-cover rounded-md shadow-sm" />
 ) : (
 <div className="w-20 h-14 bg-canvas rounded-md"></div>
 )}
 <span className="font-bold text-ink line-clamp-2">{c.title}</span>
 </div>
 ))}
 </div>
 </div>
