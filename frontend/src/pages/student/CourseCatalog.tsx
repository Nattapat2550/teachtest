...
export default function CourseCatalog() {
 ...
 {/* Wallet Status */}
 {isAuthenticated && (
 <div className="flex items-center justify-between bg-canvas p-6 rounded-md shadow-sm border border-blue-200 mb-8">
 <div>
 <h2 className="text-muted font-bold mb-1">ยอดเงินคงเหลือของคุณ</h2>
 ...
 {packages.length > 0 && (
 <div className="mb-12">
 {/* เอา text-purple-600 ออก เปลี่ยนเป็น text-ink */}
 <h1 className="text-3xl font-black mb-6 text-ink ">🔥 แพ็กเกจสุดคุ้ม</h1>
 ...
 return (
 <div key={pkg.id} className={`bg-canvas rounded-md shadow-sm border border-purple-200 p-5 flex flex-col relative transition-all duration-300 ${isPackageOwned ? 'opacity-60 grayscale hover:grayscale-0 hover:opacity-100' : 'hover:shadow-lg hover:-translate-y-1'}`}>
 ...
 <div className="aspect-video bg-canvas rounded-md mb-4 overflow-hidden">
 {pkg.cover_image ? <img src={pkg.cover_image} alt={pkg.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-muted">No Image</div>}
 </div>
 ...
 <h1 className="text-3xl font-black mb-8 text-ink ">หลักสูตรทั้งหมด</h1>
 {courses.length === 0 ? (
 <p className="text-muted">ยังไม่มีหลักสูตรที่เปิดสอน</p>
 ) : (
 ...
 key={c.id} 
 className={`bg-canvas rounded-md shadow-sm border p-4 flex flex-col relative transition-all duration-300 ${
 isOwned 
 ...
 <div className="aspect-video bg-canvas rounded-md mb-4 overflow-hidden">
 {c.cover_image ? <img src={c.cover_image} alt={c.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-muted">No Image</div>}
 </div>
