...
export default function NewsTab() {
 ...
 {/* ---------------- ฟอร์มสร้างข่าวสาร ---------------- */}
 <form onSubmit={handleCreateNews} className="bg-canvas p-8 lg:p-10 rounded-md shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-outline relative overflow-hidden w-full">
 ...
 <input type="file" name="image" accept="image/*" disabled className={`${inputStyle} file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 bg-canvas cursor-not-allowed`} title="ระบบอัปโหลดรูปกำลังอยู่ในช่วงพัฒนา" />
 </div>
 </div>
 ...
 {news.map(n => (
 <div key={String(n.id)} className="bg-canvas rounded-md border border-outline shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col relative group">
 ...
 <h4 className="font-black text-xl text-ink mb-2 line-clamp-2">{n.title}</h4>
 <p className="text-sm text-muted line-clamp-3 mt-auto">{n.content}</p>
 </div>
 <div className="p-4 bg-canvas/80 /80 border-t border-outline grid grid-cols-2 gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
 ...
 <form onSubmit={handleUpdateNews} className="bg-canvas rounded-md w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
 <div className="sticky top-0 bg-canvas z-10 px-8 py-5 border-b border-outline flex justify-between items-center">
 ...
 <button type="button" onClick={() => setEditingNews(null)} className="text-muted hover:text-ink dark:hover:text-white text-3xl leading-none">&times;</button>
 </div>
 ...
 <input type="file" name="image" accept="image/*" disabled className={`${inputStyle} bg-canvas cursor-not-allowed`} />
 </div>
 </div>
 </div>

 <div className="p-6 border-t border-outline bg-canvas /50 flex justify-end gap-3 rounded-b-3xl">
 <button type="button" onClick={() => setEditingNews(null)} className="px-6 py-3 bg-canvas border border-outline text-ink rounded-md font-bold hover:bg-canvas dark:hover:bg-gray-600 transition-colors">
 ยกเลิก
 </button>
