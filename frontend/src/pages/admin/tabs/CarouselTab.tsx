...
const inputStyle = "w-full p-3.5 bg-canvas /50 border border-outline text-ink text-sm rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-200";
const labelStyle = "block text-xs font-bold text-muted uppercase tracking-wider mb-2";
...
export default function CarouselTab() {
 ...
 <form onSubmit={handleCreate} className="bg-canvas p-8 lg:p-10 rounded-md shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-outline relative overflow-hidden w-full">
 ...
 <button type="submit" className="flex items-center gap-2 bg-linear-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-3.5 px-8 rounded-md shadow-lg shadow-blue-500/30 transition-all transform hover:-translate-y-0.5">
 ...
 {items.map(item => (
 <div key={String(item.id)} className="bg-canvas rounded-md border border-outline shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col group">
 <div className="aspect-video w-full bg-canvas relative overflow-hidden">
 ...
 <div className="absolute top-3 left-3 bg-gray-900/70 text-white px-3 py-1 rounded-md text-xs font-bold">
 Order: {item.sort_order}
 </div>
 </div>
 
 <div className="p-4 flex-1">
 <p className="text-xs text-muted truncate mb-1">Link: {item.link_url || 'None'}</p>
 </div>

 <div className="p-4 bg-canvas/80 /80 border-t border-outline grid grid-cols-2 gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
 <button onClick={() => setEditingItem(item)} className="flex items-center justify-center gap-2 py-2 bg-blue-100 hover:bg-blue-200 /40 dark:hover:bg-blue-900/60 text-blue-700 rounded-md transition-colors font-bold text-sm">
 <img src={settingsImg} className="w-4 h-4 dark:invert" alt="Edit" /> แก้ไข
 </button>
 <button onClick={() => handleDelete(item.id)} className="flex items-center justify-center gap-2 py-2 bg-red-100 hover:bg-red-200 /40 dark:hover:bg-red-900/60 text-red-700 rounded-md transition-colors font-bold text-sm">
 ...
 <form onSubmit={handleUpdate} className="bg-canvas rounded-md w-full max-w-2xl shadow-2xl flex flex-col">
 <div className="px-8 py-5 border-b border-outline flex justify-between items-center">
 ...
 <button type="button" onClick={() => setEditingItem(null)} className="text-muted hover:text-ink dark:hover:text-white text-3xl">&times;</button>
 </div>
 ...
 <div className="p-6 border-t border-outline bg-canvas /50 flex justify-end gap-3 rounded-b-3xl">
 <button type="button" onClick={() => setEditingItem(null)} className="px-6 py-3 bg-canvas border border-outline text-ink rounded-md font-bold hover:bg-canvas dark:hover:bg-gray-600 transition-colors">
 ยกเลิก
 </button>
