...
export default function ShopPage() {
  ...
  if (!shop) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-canvas  text-gray-900 ">
      ...
      <div className="px-6 lg:px-12 2xl:px-20 mt-10">
          ...
          <div className="flex gap-4 w-full md:w-auto">
            <div className="relative w-full md:w-80">
              <input 
                ...
                className="w-full pl-11 pr-4 py-3.5 rounded-md border border-gray-200  bg-white  text-gray-900  focus:ring-2 focus:ring-orange-500 outline-none shadow-sm transition-all"
              />
        ...
        {filteredProducts.length === 0 ? (
           <div className="text-center py-24 bg-white  rounded-md border border-gray-200  shadow-sm">
             ...
             <p className="text-gray-500  font-medium">ลองค้นหาด้วยคำอื่น หรือร้านอาจจะยังไม่มีสินค้า</p>
           </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filteredProducts.map(p => (
              <Link to={`/products/${p.id}`} key={p.id} className="group bg-white  rounded-md shadow-sm border border-gray-200  overflow-hidden hover:shadow-2xl hover:shadow-orange-500/10 hover:-translate-y-1.5 transition-all duration-300 flex flex-col h-full">
