...
export default function TutorDashboard() {
 ...
 const fileInputClass = "mb-4 block w-full text-sm text-ink border border-outline rounded-md cursor-pointer bg-canvas focus:outline-none dark:placeholder-gray-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-l-lg file:border-0 file:text-sm file:font-bold file:bg-canvas file:text-ink hover:file:bg-gray-300 dark:file:bg-gray-600 dark:file:text-white transition-all";
 ...
 {/* Tabs Menu */}
 <div className="flex gap-2 border-b border-outline mb-8 overflow-x-auto">
 <button onClick={()=>setActiveTab('manage_courses')} className={`py-3 px-6 font-bold whitespace-nowrap ${activeTab==='manage_courses' ? 'text-primary border-b-2 border-blue-600' : 'text-muted hover:text-ink '}`}>จัดการคอร์สเรียน</button>
 <button onClick={()=>setActiveTab('content')} className={`py-3 px-6 font-bold whitespace-nowrap ${activeTab==='content' ? 'text-primary border-b-2 border-blue-600' : 'text-muted hover:text-ink '}`}>จัดการเนื้อหา & โปรโมโค้ด</button>
 <button onClick={()=>setActiveTab('packages')} className={`py-3 px-6 font-bold whitespace-nowrap ${activeTab==='packages' ? 'text-primary border-b-2 border-blue-600' : 'text-muted hover:text-ink '}`}>จัดแพ็กเกจรวมคอร์ส</button>
 <button onClick={()=>setActiveTab('analytics')} className={`py-3 px-6 font-bold whitespace-nowrap ${activeTab==='analytics' ? 'text-primary border-b-2 border-blue-600' : 'text-muted hover:text-ink '}`}>📊 สถิติและข้อมูลหลังบ้าน</button>

 {role === 'admin' && (
 <button onClick={()=>setActiveTab('global_promos')} className={`py-3 px-6 font-bold whitespace-nowrap ${activeTab==='global_promos' ? 'text-primary border-b-2 border-blue-600' : 'text-muted hover:text-ink '}`}>โค้ดส่วนลดกลาง (Admin)</button>
 )}
 </div>
