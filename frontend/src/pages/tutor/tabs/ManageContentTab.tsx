...
export default function ManageContentTab(props: ManageContentProps) {
 ...
 return (
 <div className="bg-canvas p-8 rounded-md shadow-sm border border-outline transition-colors">
 <div className="mb-8">
 <h2 className="text-2xl font-black mb-4 text-ink ">จัดการเนื้อหาและโปรโมโค้ด</h2>
 <select 
 className="w-full p-4 border border-outline rounded-md outline-none focus:ring-2 focus:ring-blue-500 transition-all" 
 onChange={(e) => {
 ...
 <input type="number" placeholder="ลำดับ" required className="w-full sm:w-24 p-3.5 border border-outline rounded-md outline-none focus:ring-2 focus:ring-purple-500" value={playlistForm.sort_order} onChange={e=>setPlaylistForm({...playlistForm, sort_order: Number(e.target.value)})} />
 <input type="text" placeholder="ชื่อบทเรียน" required className="flex-1 p-3.5 border border-outline rounded-md outline-none focus:ring-2 focus:ring-purple-500" value={playlistForm.title} onChange={e=>setPlaylistForm({...playlistForm, title: e.target.value})} />
 ...
 {editingPlaylistId && <button type="button" onClick={()=>{setEditingPlaylistId(null); setPlaylistForm({title: '', sort_order: 1});}} className="bg-canvas text-ink px-6 py-3.5 font-bold rounded-md hover:bg-gray-300 transition-colors">ยกเลิก</button>}
 </div>
 </form>
 </div>
 ...
 <div key={pl.id} className="bg-canvas /50 p-6 rounded-md border border-outline ">
 ...
 <div key={it.id}>
 {editingItemId === it.id ? (
 <div className="p-6 bg-canvas border-2 border-blue-400 rounded-md my-4 shadow-sm">
 <div className="flex flex-col sm:flex-row gap-3 mb-6">
 <input type="number" placeholder="ลำดับ" required className="w-full sm:w-24 p-3 border border-outline rounded-md outline-none" value={itemForm.sort_order} onChange={e=>setItemForm({...itemForm, sort_order: Number(e.target.value)})} />
 <input type="text" placeholder="ชื่อคลิป/ไฟล์" required className="flex-1 p-3 border border-outline rounded-md outline-none" value={itemForm.title} onChange={e=>setItemForm({...itemForm, title: e.target.value})} />
 <select className="p-3 border border-outline rounded-md outline-none font-medium" value={itemForm.item_type} onChange={e=>setItemForm({...itemForm, item_type: e.target.value})}>
 ...
 {uploading && (
 <div className="w-full bg-canvas rounded-full h-3 mb-6 overflow-hidden">
 ...
 <button onClick={() => setEditingItemId(null)} disabled={uploading} className="bg-canvas hover:bg-canvas text-ink px-8 py-3 rounded-md font-bold transition-colors">ยกเลิก</button>
 </div>
 </div>
 ) : (
 <div className="flex justify-between items-center bg-canvas p-4 rounded-md border border-outline hover:shadow-sm transition-shadow">
 <span className=" flex items-center gap-3">
 <span className="text-muted bg-canvas w-6 h-6 flex items-center justify-center rounded text-xs font-bold">{it.sort_order}</span>
 ...
 }} className="w-full border-2 border-dashed border-outline text-primary hover:border-blue-400 hover:bg-blue-50/50 dark:hover:bg-gray-800 py-4 font-bold rounded-md transition-all">
