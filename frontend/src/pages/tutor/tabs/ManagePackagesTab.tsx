...
export default function ManagePackagesTab({
 ...
 <div className="border border-outline rounded-md p-4 bg-canvas h-40 overflow-y-auto">
 <p className="text-sm font-bold text-ink mb-2">เลือกคอร์สที่จะรวมในแพ็ก:</p>
 {courses.map((c: any) => (
 ...
 {editingPkgId && <button type="button" onClick={()=>{setEditingPkgId(null); setPkgForm({title:'',description:'',price:0,cover_image:'',course_ids:[], is_published: true, access_duration_days: null})}} className="px-4 bg-canvas text-ink font-bold rounded-md">ยกเลิก</button>}
 </div>
 </form>
 </div>
 ...
 {packages.map((p: any) => (
 <div key={p.id} className={`bg-canvas rounded-md border shadow-sm p-4 ${p.is_published ? 'border-outline ' : 'border-red-300 opacity-60'}`}>
 ...
 <p className="text-sm text-muted mb-3">{p.course_ids?.length || 0} คอร์สที่รวมอยู่ {p.access_duration_days ? `(เรียนได้ ${p.access_duration_days} วัน)` : ''}</p>
 <div className="flex gap-2">
