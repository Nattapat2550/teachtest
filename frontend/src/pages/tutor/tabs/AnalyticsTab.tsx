    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="bg-white  rounded-md shadow-sm border border-gray-100  p-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-canvas /50 text-gray-600  text-sm">
      ...
      <div className="bg-white  rounded-md shadow-sm border border-gray-100  p-6">
        ...
        <div className="overflow-x-auto max-h-80">
          <table className="w-full text-left border-collapse">
            <thead className="bg-canvas /50 text-gray-600  text-sm sticky top-0">
              <tr>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {data.recent_sales.map((s: any) => (
                <tr key={s.id} className="hover:bg-canvas dark:hover:bg-gray-700/50 text-sm">
                  <td className="p-3 text-gray-500 ">{new Date(s.enrolled_at).toLocaleString('th-TH')}</td>
                  <td className="p-3 font-mono text-gray-600 ">{s.student_id.substring(0,8)}...</td>
                  <td className="p-3 font-semibold text-gray-900 ">{s.course_title}</td>
                  <td className="p-3">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {data.promo_usages.map((p: any, idx: number) => (
                <tr key={idx} className="hover:bg-canvas dark:hover:bg-gray-700/50 text-sm">
                  <td className="p-3 text-gray-500 ">{new Date(p.used_at).toLocaleString('th-TH')}</td>
                  <td className="p-3"><span className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold">{p.code}</span></td>
                  <td className="p-3 font-mono text-gray-600 ">{p.student_id.substring(0,8)}...</td>
                  <td className="p-3 font-semibold text-gray-900 ">{p.course_title}</td>
                </tr>
              ))}
