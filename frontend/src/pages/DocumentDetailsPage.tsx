  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-canvas  flex justify-center items-center">
        <div className="text-xl font-bold text-gray-900 ">กำลังโหลดข้อมูล...</div>
      </div>
    );
  }
  ...
  if (!doc) {
    return (
      <div className="min-h-screen bg-canvas  flex justify-center items-center flex-col">
        <div className="text-xl font-bold text-gray-900  mb-4">ไม่พบข้อมูลที่คุณค้นหา</div>
  ...
  return (
    <div className="w-full min-h-screen bg-canvas  pb-20 pt-24 px-6 md:px-12 transition-colors duration-300">
      <div className="max-w-7xl w-full mx-auto">
        ...
        <Link to="/" className="inline-flex items-center text-primary  font-bold hover:underline mb-8 bg-white  px-5 py-2.5 rounded-full border border-gray-200  shadow-sm transition-colors">
          &larr; กลับหน้าหลัก
        </Link>

        {/* ภาพปก */}
        {doc.cover_image && (
          <div className="w-full h-75 md:h-125 rounded-md overflow-hidden mb-10 shadow-lg border border-gray-200  bg-white ">
        ...
        {gallery.length > 0 && (
          <div className="mt-16 pt-10 border-t border-gray-200 ">
            <h2 className="text-3xl font-black text-gray-900  mb-8">แกลเลอรีรูปภาพ</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {gallery.map((url, idx) => (
                <div key={idx} className="group rounded-md overflow-hidden shadow-sm bg-white  border border-gray-200  hover:shadow-xl hover:border-primary/30 transition-all cursor-pointer">
                  <img 
                    src={url} 
                    alt={`Gallery ${idx + 1}`} 
