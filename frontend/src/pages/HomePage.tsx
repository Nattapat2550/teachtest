    ...
    <div className="w-full overflow-x-hidden bg-gray-50 dark:bg-gray-900 pb-20 transition-colors duration-300">
        ...
        <div className="w-full px-6 lg:px-12 2xl:px-20 py-24 lg:py-32 relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="text-center md:text-left flex-1">
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-semibold mb-6 leading-tight tracking-tighter">
              ...
              <span className="text-gray-400 dark:text-gray-500">Learning</span> Journey
            </h1>
            ...
            <Link to="/courses" className="inline-flex items-center gap-2 bg-black dark:bg-white text-white dark:text-black font-semibold py-4 px-10 rounded-full hover:scale-105 transition-transform duration-300">
      ...
      {isCoursesLoading ? (
        <div className="w-full px-6 lg:px-12 2xl:px-20 mt-16 md:mt-24">
           <div className="h-10 w-48 bg-gray-200 dark:bg-gray-800 rounded-lg animate-pulse mb-10"></div>
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
             {[1,2,3,4].map(i => <div key={i} className="h-80 bg-gray-200 dark:bg-gray-800 rounded-[2rem] animate-pulse"></div>)}
           </div>
        </div>
      ) : courses.length > 0 && (
        <div className="w-full px-6 lg:px-12 2xl:px-20 mt-16 md:mt-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {courses.map((c: any) => (
              <Link to={`/courses/${c.id}`} key={c.id} className="group apple-glass rounded-[2rem] overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col">
                <div className="h-56 bg-gray-100 dark:bg-gray-900 overflow-hidden relative">
                ...
                <div className="p-6 flex flex-col flex-1 bg-white/50 dark:bg-black/50">
                  <h3 className="text-xl font-semibold mb-2 text-gray-900 dark:text-white line-clamp-2 tracking-tight">{c.title}</h3>
                  <div className="mt-auto pt-4 flex justify-between items-center">
                    <span className="text-xl font-semibold text-gray-900 dark:text-white">฿ {Number(c.price).toLocaleString()}</span>
                    <span className="bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white px-4 py-2 rounded-full text-sm font-medium">รายละเอียด</span>
                  </div>
                </div>
              </Link>
            ))}
      ...
      {isDocumentsLoading ? null : documents.length > 0 && (
        <div className="w-full px-6 lg:px-12 2xl:px-20 mt-16 md:mt-24">
          <div className="flex items-center gap-4 mb-10">
            ...
            <h2 className="text-3xl md:text-4xl font-semibold text-gray-900 dark:text-white tracking-tight">เอกสารและบทความ</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {documents.map((d: any) => (
              <div key={d.id} className="group apple-glass rounded-[2rem] overflow-hidden flex flex-col hover:shadow-xl transition-all duration-300">
                ...
                <div className="p-6 bg-white/50 dark:bg-black/50 flex flex-col flex-1">
                  <h3 className="text-lg font-semibold mb-4 dark:text-white line-clamp-2 tracking-tight">{d.title}</h3>
                  <Link to={`/documents/${d.id}`} className="block text-center mt-auto w-full bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-900 dark:text-white font-medium py-3 rounded-full transition-colors">อ่านต่อ</Link>
                </div>
              </div>
            ))}
          </div>
