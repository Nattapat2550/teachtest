...
export default function HomePage() {
 ...
 <span className="text-muted ">Learning</span> Journey
 </h1>
 ...
 <div className="w-full h-62.5 md:h-100 lg:h-112.5 rounded-[2rem] bg-canvas animate-pulse"></div>
 </div>
 ) : carousels.length > 0 && (
 <div className="w-full px-6 lg:px-12 2xl:px-20 mt-10">
 ...
 <div className="h-10 w-48 bg-canvas rounded-md animate-pulse mb-10"></div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
 {[1,2,3,4].map(i => <div key={i} className="h-80 bg-canvas rounded-[2rem] animate-pulse"></div>)}
 </div>
 </div>
 ) : courses.length > 0 && (
 <div className="w-full px-6 lg:px-12 2xl:px-20 mt-16 md:mt-24">
 <div className="flex items-center justify-between mb-10">
 <h2 className="text-3xl md:text-4xl font-semibold text-ink tracking-tight flex items-center gap-3">
 <BookOpen className="text-muted" size={32} /> คอร์สเรียนแนะนำ
 </h2>
 <Link to="/courses" className="text-muted hover:text-ink dark:hover:text-white font-medium flex items-center transition-colors">ดูทั้งหมด <ChevronRight size={20}/></Link>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
 {courses.map((c: any) => (
 ...
 <div className="h-56 bg-canvas overflow-hidden relative">
 {c.cover_image ? <img src={c.cover_image} alt="Cover" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"/> : <div className="w-full h-full flex items-center justify-center text-muted">No Image</div>}
 ...
 <h3 className="text-xl font-semibold mb-2 text-ink line-clamp-2 tracking-tight">{c.title}</h3>
 <div className="mt-auto pt-4 flex justify-between items-center">
 <span className="text-xl font-semibold text-ink ">฿ {Number(c.price).toLocaleString()}</span>
 <span className="bg-canvas text-ink px-4 py-2 rounded-full text-sm font-medium">รายละเอียด</span>
 </div>
 </div>
 </Link>
 ))}
 ...
 <h2 className="text-3xl md:text-4xl font-semibold text-ink tracking-tight">เอกสารและบทความ</h2>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
 {documents.map((d: any) => (
 ...
 <div className="h-48 bg-canvas overflow-hidden">
 ...
 <div className="p-6 bg-canvas/50 /50 flex flex-col flex-1">
 <h3 className="text-lg font-semibold mb-4 line-clamp-2 tracking-tight">{d.title}</h3>
 <Link to={`/documents/${d.id}`} className="block text-center mt-auto w-full bg-canvas hover:bg-canvas dark:hover:bg-gray-700 text-ink font-medium py-3 rounded-full transition-colors">อ่านต่อ</Link>
 </div>
 </div>
 ))}
 </div>
