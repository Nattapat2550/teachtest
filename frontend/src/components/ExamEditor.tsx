...
export default function ExamEditor({ examQuestions, setExamQuestions }: ExamEditorProps) {
 ...
 <div key={qIdx} className="p-5 bg-canvas rounded-md shadow-sm border border-outline transition-all">
 <div className="flex justify-between items-center mb-4">
 <span className="font-bold text-muted">ข้อ {qIdx + 1}</span>
 <select 
 value={q.question_type || 'multiple_choice'} 
 onChange={(e) => updateQuestion(qIdx, 'question_type', e.target.value)}
 className="p-2 border border-outline rounded-md outline-none font-bold text-sm bg-canvas focus:ring-2 focus:ring-blue-500"
 >
 ...
 className="w-full p-3 mb-4 border border-outline rounded-md outline-none focus:ring-2 focus:ring-blue-500"
 />
 
 <div className="mb-6 p-4 bg-canvas /50 rounded-md border border-outline ">
 <label className="block text-sm font-bold text-muted mb-2">แนบรูปภาพหรือวิดีโอ (ถ้ามี)</label>
 ...
 <video src={getFullUrl(q.image_url)} controls className="max-w-xs rounded-md shadow-sm border border-outline " />
 ) : (
 <img src={getFullUrl(q.image_url)} alt="Media" className="max-w-xs rounded-md shadow-sm border border-outline " />
 )}
 ...
 <label className="block text-sm font-bold text-muted mb-2">ตัวเลือก (เลือกข้อที่ถูก)</label>
 ...
 className={`flex-1 border rounded-md p-3 outline-none focus:ring-2 transition-all ${c.is_correct ? 'border-primary ring-1 ring-blue-500 bg-blue-50 /20' : 'border-outline '}`}
 />
 {q.choices.length > 2 && (
 ...
 <div>
 <label className="block text-sm font-bold text-muted mb-2">คำตอบที่ถูกต้อง (ซ่อนได้)</label>
 <input 
 type="text" 
 placeholder="พิมพ์คำตอบที่ถูกต้อง..." 
 value={q.correct_answer || ''} 
 onChange={(e) => updateQuestion(qIdx, 'correct_answer', e.target.value)}
 className="w-full p-3 border border-outline rounded-md outline-none focus:ring-2 focus:ring-blue-500"
 />
 </div>
 )}
 
 {examQuestions.length > 1 && (
 <div className="mt-6 pt-4 border-t border-outline text-right">
 <button type="button" onClick={() => {
 const newQs = [...examQuestions];
 newQs.splice(qIdx, 1);
 setExamQuestions(newQs);
