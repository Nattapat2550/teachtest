...
export default function NewsPopup() {
 ...
 <div className="bg-canvas p-6 rounded-md max-w-2xl w-full relative max-h-[90vh] flex flex-col">
 <button 
 onClick={closeNewsModal} 
 className="absolute top-4 right-4 text-muted hover:text-ink text-3xl font-bold leading-none"
 >
 &times;
 </button>
 ...
 <h3 className="text-xl font-bold mb-2 text-ink">{news.title}</h3>
 <p className="text-ink mb-4 leading-relaxed whitespace-pre-line">{news.content}</p>
 {news.image_url && (
 ...
 className="w-4 h-4 text-primary rounded border-outline focus:ring-blue-500"
 />
 <span className="text-ink text-sm font-medium">ไม่แสดงข่าวสารนี้อีก (สำหรับประกาศรอบนี้)</span>
 </label>
