...
func (h *Handler) CenterGetDashboard(w http.ResponseWriter, r *http.Request) {
	...
	err := h.TeachDB.QueryRow("SELECT id, name FROM delivery_centers WHERE center_user_id = $1", uidStr).Scan(&center.ID, &center.Name)
	if err != nil {
...
func (h *Handler) CenterUpdateProfile(w http.ResponseWriter, r *http.Request) {
	...
	var centerID string
	err := h.TeachDB.QueryRow("SELECT id FROM delivery_centers WHERE center_user_id = $1", uidStr).Scan(&centerID)

	switch err {
	case sql.ErrNoRows:
		_, err = h.TeachDB.Exec("INSERT INTO delivery_centers (center_user_id, name) VALUES ($1, $2)", uidStr, req.Name)
	case nil:
		_, err = h.TeachDB.Exec("UPDATE delivery_centers SET name = $1 WHERE id = $2", req.Name, centerID)
	}
...
func (h *Handler) CenterGetRiders(w http.ResponseWriter, r *http.Request) {
	...
	err := h.TeachDB.QueryRow("SELECT id FROM delivery_centers WHERE center_user_id = $1", uidStr).Scan(&centerID)
	if err != nil {
		http.Error(w, "Center not found", http.StatusForbidden)
		return
	}

	rows, err := h.TeachDB.Query("SELECT id, rider_user_id, created_at FROM riders WHERE center_id = $1", centerID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) CenterAddRider(w http.ResponseWriter, r *http.Request) {
	...
	_, err = h.TeachDB.Exec(`
		INSERT INTO riders (rider_user_id, center_id)
		VALUES ($1, $2)
		ON CONFLICT (rider_user_id)
...
func (h *Handler) CenterRemoveRider(w http.ResponseWriter, r *http.Request) {
	...
	_, err = h.TeachDB.Exec("DELETE FROM riders WHERE id = $1 AND center_id = $2", id, centerID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) CenterBatchAssign(w http.ResponseWriter, r *http.Request) {
	...
	for _, sid := range req.ShipmentIDs {
		_, err := h.TeachDB.Exec("UPDATE shipments SET status='delivering', rider_id=$1, updated_at=CURRENT_TIMESTAMP WHERE id=$2 AND current_center_id=$3 AND status='at_center'", req.RiderID, sid, center.ID)
		if err == nil {
			var orderID string
			h.TeachDB.QueryRow("SELECT order_id FROM shipments WHERE id=$1", sid).Scan(&orderID)
			h.TeachDB.Exec("INSERT INTO order_tracking (order_id, status_detail, location) VALUES ($1, $2, $3)", orderID, "จัดสรรไรเดอร์แล้ว (Batch)", center.Name)
		}
	}
