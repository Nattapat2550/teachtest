...
func setupLMSRoutes(h *handlers.Handler) func(chi.Router) {
	return func(r chi.Router) {
		...
		r.Get("/packages", h.GetPublishedPackages)
		r.Get("/packages/{id}", h.GetPackageDetail) // เปิด Route สำหรับดึงรายละเอียดแพ็กเกจ

		r.Group(func(r chi.Router) {
			r.Use(h.RequireAuth)

			r.Post("/student/enroll", h.StudentEnrollCourse)
			r.Post("/student/enroll-package", h.StudentEnrollPackage)
